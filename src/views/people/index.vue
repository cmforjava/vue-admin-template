  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="类型">
        <template slot-scope="scope">
          {{ scope.row.mailing_type_str }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="是否代收款">
        <template slot-scope="scope">
          {{ scope.row.is_agency_fund_str }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="代收款">
        <template slot-scope="scope">
          {{ scope.row.amount_agency_fund }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="运费">
        <template slot-scope="scope">
          {{ scope.row.amount_freight }}
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="取件快递员">
        <template slot-scope="scope">
          {{ scope.row.created_at }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="收件奖励">
        <template slot-scope="scope">
          {{ scope.row.created_at }}
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleEdit(scope) {
      this.$router.push('/setting/permissionedit/' + scope.row.id)
    },
    handleCreate() {
      console.log(1)
    },
